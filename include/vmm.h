...
// 获取一个地址的页目录项
#define PGD_INDEX(x) (((x) >> 22) & 0x3FF) // 一个地址中31 到22 位表示的目录项 一共10位

// 获取一个地址的页表项
#define PTE_INDEX(x) (((x) >> 12) & 0x3FF) //从12位到21位这10位用来表示页面表

// 获取一个地址的页內偏移
#define OFFSET_INDEX(x) ((x) & 0xFFF)  // 0到11位 这12位用来表示偏移
